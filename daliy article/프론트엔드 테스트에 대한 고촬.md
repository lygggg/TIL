...
위 예시에서는 태그의 Backspace를 누르면 해당 태그가 삭제되는 것을 테스트한 코드입니다. 우리는 저 테스트 코드만 보고도 어떤 역할을 하는 컴포넌트인지 쉽고 빠르게 파악할 수 있습니다. 빠르게 파악이 가능 하다는 것은 클린 코드에서 계속해서 강조하는 변수, 함수 네이밍과 비슷하게 큰 이점이 된다고 생각합니다.
...
아래에서 RTL의 간단한 사용법말고 더 나은 테스트를 작성하는 방법에 대해서 이야기해보겠습니다.

### 쿼리 우선순위
공식문서를 확인하면 우선순위대로 사용하는 것이 좋다고 말합니다. `getByRole`, `getByLabelText`, `getByPlaceholderText`, `getByText` 등등 있습니다.

저는 React Testing Library를 처음 사용했을 때, getByTestId를 주로 사용했습니다. 정말 편하기 때문이죠.. 일일히 생각할 필요도 없이 `data-testid`만 추가하면 쉽게 접근할 수 있었습니다. 하지만 getByTestId는 공식문서에 나와있는 쿼리 우선순위들 중에서 가장 낮은 우선순위를 가지고 있습니다. 왜 그런걸까요? 위에서 말했던 것 처럼 우리의 사용자는 구성 요소의 세부 정보를 알지 못하고, 눈에 보이는 버튼을 클릭할 뿐입니다. 그래서 결론적으로 우리는 사용자 관점에서 테스트를 작성할 필요가 있습니다.


### 쿼리 성능
그럼 무조건 우리는 getByRole를 사용해야 할까요? RTL을 사용해본 사람들중 일부분은 `getByRole()`이 다른 `getBy`쿼리보다 성능이 떨어진다고 말합니다. [해당 이슈](https://github.com/testing-library/dom-testing-library/issues/820) 해당 이슈들을 확인해보면, 정확한 수치는 아니지만 대략 5배의 차이를 보인다고합니다. 그래서 무조건 `getByRole()`이 맞다 라는 생각보단 상황에 맞게 적절한 쿼리를 사용하는게 중요합니다. 테스트를 실행하는 시간도 비용이니까요! 자신이 만약 CI를 구축해서 테스트를 자동화한다고 하면 더더욱 중요합니다.
근데 왜 `getByRole()`은 다른 쿼리에 비해서 느린걸까요..? 이 문제에 대해서 정확하게 설명하는 글이 없어서 저도 제대로 알지는 못합니다. 그래도 제가 생각하기에는 `getByRole`이 해당 속성이 있는 요소를 전체 DOM에서 검색하기 때문에 다른 쿼리들 보다 느린것이 아닌가 싶습니다. 당연히 check하는 DOM이 많은 경우 오래 걸리는 것이 당연하니까요. 물론 제 추측일 뿐입니다.ㅎㅎ

### 알아보기 쉬운 테스트 작성하기
어떤 테스트가 알아보기 쉬운 테스트일까요? 제가 생각했을때는 테스트를 계층구조로 작성하면 개발자가 아닌 사람이 봐도 쉽게 테스트를 이해할 수 있을 것이라고 판단했습니다. 
...
어떤가요? 계층 구조로 테스트를 작성했기 때문에 훨씬 이해하기 쉽지않나요? 비교적 작은 기능을 테스트했지만, 더 복잡한 기능을 테스트할 경우 효과는 더 커질 것이라고 생각합니다.
